...
export async function GET() {
  const greenhouseBoard =
    (Array.isArray(boards) && boards[0]?.board) || "stripe";
  ...
  const teamtailorSub =
    (sources.teamtailor && sources.teamtailor[0]) || "example";
  ...
  const toStatus = (r: any): Status =>
    r.ok
      ? {
          ok: true,
          count: (r.value as MinimalJob[]).length,
          sample: (r.value as MinimalJob[])[0] ?? null,
          tookMs: r.tookMs,
        }
